...
// Synthetic Bangladesh traffic identities for the detection overlay. These are
// display-only examples, not BRTA records or personal vehicle information.
const BANGLADESH_TRAFFIC_REGISTRATIONS = [
  ['ঢাকা মেট্রো', ['482731 CAR', '705294 CAR', '361847 BUS', '924615 TRUCK', '538207 BIKE', '617483 CAR', '250936 BIKE', '843751 TRUCK', '496208 CAR', '731564 BUS', '285947 BIKE', '604821 CAR', '917350 TRUCK', '358614 CAR', '782405 BIKE', '429761 BUS', '850327 CAR', '316582 TRUCK', '694173 BIKE', '527408 CAR', '861295 BUS', '473906 TRUCK', '718524 CAR', '392761 BIKE', '645218 CAR']],
  ['গাজীপুর মেট্রো', ['583047 CAR', '926415 TRUCK', '314782 BUS', '750631 CAR', '468295 BIKE', '831704 CAR', '295618 TRUCK', '617530 BUS', '409826 CAR', '752184 BIKE', '386507 CAR', '941263 TRUCK', '528719 BUS', '670435 CAR', '813926 BIKE', '254781 CAR', '639508 TRUCK', '475216 BUS', '908341 CAR', '351697 BIKE', '724580 CAR', '586214 TRUCK', '830765 BUS', '412953 CAR', '697128 BIKE']],
  ['নারায়ণগঞ্জ মেট্রো', ['584027 CAR', '936415 TRUCK', '317682 BUS', '750294 CAR', '468531 BIKE', '821763 CAR', '295047 TRUCK', '613825 BUS', '407216 CAR', '752963 BIKE', '386214 CAR', '941507 TRUCK', '528361 BUS', '670924 CAR', '813475 BIKE', '254690 CAR', '639817 TRUCK', '475308 BUS', '908526 CAR', '351742 BIKE', '724913 CAR', '586730 TRUCK', '830241 BUS', '412685 CAR', '697354 BIKE']],
].flatMap(([area, entries]) => entries.map((entry) => {
  const [number, type] = entry.split(' ');
  return { area, number, type };
}));

function trafficIdentity(index) {
  const identity = BANGLADESH_TRAFFIC_REGISTRATIONS[index % BANGLADESH_TRAFFIC_REGISTRATIONS.length];
  return {
    id: `${identity.area} ${identity.number}`,
    registration: `${identity.area} ${identity.number}`,
    vehicleType: identity.type,
  };
}
/**
...
const trafficLayer = {
  ...
  getDetectableObjects(options = {}) {
    ...
    for (let i = start; i < _dots.length; i += stride) {
      ...
      if (!pos) continue;
      const identity = trafficIdentity(i);
      const entry = {
        position: pos,
        id: identity.id,
