...
test('city presets include Bangladesh and Vietnam locations', async () => {
  const { CITY_POIS } = await import('./locations.js');
  assert.ok(CITY_POIS.dhaka, 'Dhaka preset should exist');
  assert.ok(CITY_POIS.hanoi, 'Hanoi preset should exist');
  assert.ok(CITY_POIS.ho_chi_minh, 'Ho Chi Minh City preset should exist');
  assert.equal(CITY_POIS.dhaka.name, 'Dhaka');
  assert.equal(CITY_POIS.hanoi.name, 'Hanoi');
  assert.equal(CITY_POIS.ho_chi_minh.name, 'Ho Chi Minh City');
});
