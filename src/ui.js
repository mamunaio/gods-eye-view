...
export class StyleManager {
  ...
  _syncCctvSourceBadge(activeCamera, enabled) {
    ...
    const rawKind = String(activeCamera.sourceKind || activeCamera.feedType || 'unknown').toLowerCase();
    const kind = rawKind === 'demo' ? 'CLIP' : rawKind.toUpperCase();
  ...
  _renderCctvState(state) {
    ...
    const activeVideo = ['mp4', 'webm', 'hls'].includes(String(activeCamera?.feedType || '').toLowerCase());
    if (effectiveActiveId
      && effectiveActiveId !== this._lastSeenCctvActiveId
      && (!state?.autoHop || isFirstActivation || activeVideo)) {
    ...
    if (this._cctvMeta) {
      if (activeCamera) {
        const provider = String(activeCamera.sourceLabel || activeCamera.provider || 'Configured Source')
          .replace(/\bdemo\b/gi, 'clip');
    ...
    const isVideo = enabled && ['mp4', 'webm', 'hls'].includes(String(activeCamera?.feedType || '').toLowerCase());
    if (this._cctvVideo) {
      const nextCameraId = enabled ? (activeCamera?.id || '') : '';
      const nextMedia = isVideo ? activeCamera?.mediaUrl : '';
      const videoChanged = this._cctvVideo.dataset.cameraId !== nextCameraId
        || this._cctvVideo.dataset.currentSrc !== nextMedia;
      if (videoChanged) {
        this._cctvVideo.dataset.cameraId = nextCameraId;
        this._cctvVideo.dataset.currentSrc = nextMedia || '';
        this._cctvVideo.classList.toggle('active', Boolean(nextMedia));
        if (nextMedia) {
          this._cctvVideo.src = nextMedia;
