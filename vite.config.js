...
/** Default Dhaka demo pack, merged with the live provider catalogs. */
const DEFAULT_DHAKA_SOURCE_FILE = 'config/cctv_sources.dhaka.json';
/** Public CC0 clip used when a configured demo camera needs visible media. */
const PUBLIC_CCTV_CLIP_URL = 'https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4';
...
function loadSourcesFromFile() {
  const sourceFiles = process.env.CCTV_SOURCES_FILE
    ? [process.env.CCTV_SOURCES_FILE]
    : [DEFAULT_CCTV_SOURCE_FILE, DEFAULT_DHAKA_SOURCE_FILE];
  const sources = [];
...
function normalizeSourceItem(item) {
  const isDemoClip = String(item.sourceKind || item.kind || '').toLowerCase() === 'demo';
  const isDhakaSource = String(item.cityId || '').trim().toLowerCase() === 'dhaka';
  const cleanText = (value) => String(value || '').replace(/\bdemo\b/gi, 'clip');
  return {
    id: String(item.id || '').trim(),
    name: cleanText(item.name || item.id).trim(),
    city: String(item.city || ''),
    cityId: String(item.cityId || ''),
    provider: cleanText(item.provider || 'Configured CCTV Source'),
    ...
    // Dhaka entries use Google Street View location frames through the existing
    // server-side fallback; no generic sample clip is shown as road footage.
    feedType: isDhakaSource ? 'image' : normalizeFeedType(item.feedType || item.type || ''),
    url: isDhakaSource ? '' : (isDemoClip ? PUBLIC_CCTV_CLIP_URL : (typeof item.url === 'string' ? item.url : '')),
    snapshotUrl: typeof item.snapshotUrl === 'string' ? item.snapshotUrl : '',
    license: cleanText(item.license || item.licenseNote || ''),
    sourceKind: isDhakaSource
      ? 'streetview'
      : isDemoClip ? 'clip' : String(item.sourceKind || item.kind || 'configured'),
...
async function refreshCctvSources() {
  ...
  const operatorConfiguredSources = Boolean(
    String(process.env.CCTV_SOURCES_FILE || '').trim()
      || String(process.env.CCTV_SOURCES_JSON || '').trim(),
  );
  const needsLiveSources = forceAustin || (!operatorConfiguredSources && preferAustin);
